...
CR_3D <- scatterplot3d(ID_AUMETO,pib_base_AUMETO$`log_pibpct-1960`,Mean_anual_AUMETO, color =  'red')

#Etiquetas por país

text(pib_base_AUMETO$`log_pibpct-1960`,Mean_anual_AUMETO,AUMETO$`Country Code`,cex = 0.6,pos = 4,col = "red")
fit <- lm(Mean_anual_AUMETO ~ ID_AUMETO+pib_base_AUMETO$`log_pibpct-1960`)
CR_3D$plane3d(fit)

#Agregar barras a los puntos y guardar gráfica

G3D <- scatterplot3d(ID_AUMETO,pib_base_AUMETO$`log_pibpct-1960`,Mean_anual_AUMETO,pch = 16,highlight.3d = TRUE,type = "h",main = "Convergencia Relativa \n AUMETO ",xlab = "País", ylab = "1961",zlab = "Tasa crecimiento")

#AGREGAR NOMBRES A LOS PUNTOS

text(G3D$xyz.convert(ID_AUMETO,pib_base_AUMETO$`log_pibpct-1960`,Mean_anual_AUMETO),labels = AUMETO$PAÍS,cex = 0.1,col ="steelblue")
